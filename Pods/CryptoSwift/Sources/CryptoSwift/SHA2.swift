...
final class SHA2 : HashProtocol {
    ...
    func calculate64() -> [UInt8] {
        ...
        variant.resultingArray(hh).forEach {
            let item = $0.bigEndian
            let one = UInt8(item & 0xff)
            let two = UInt8((item >> 8) & 0xff)
            let three = UInt8((item >> 16) & 0xff)
            let four = UInt8((item >> 24) & 0xff)
            let five = UInt8((item >> 32) & 0xff)
            let six = UInt8((item >> 40) & 0xff)
            let seven = UInt8((item >> 48) & 0xff)
            let eight = UInt8((item >> 56) & 0xff)
            let array = [
                one,
                two,
                three,
